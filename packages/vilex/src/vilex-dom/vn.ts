...
import { g } from "./g";
import { isCssKey } from "./utils/isCssKey";
...
export function vn( tag: string, options: VnItem[]) {
  ...
  let vnode = DataNode(dataModel) as VNode;
  vnode.$ex = g.ex ;
  g.ex = ''
  vnode.el = element(tag);
  vnode.on(EmitType.ON_NODE_CHANGE, (eventType: string | number, ...args: any[]) =>

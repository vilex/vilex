...
export function regRoute(path:string, component: RouteComponent, root = false) {
...
function matchedPath(path: string) {
    return Array.from(routeMap.keys()).filter(key => path.includes(key)).sort((a:string, b:string) => a.length - b.length)
}
